...
import { SkeletonCard, SkeletonKpiRow, SkeletonTable } from './ui/Skeleton.jsx';
...
export default function ProductionWorkspace({ profile, onOpenAttendance, onOpenPanel, onOpenBilling }) {
  ...
  const navigationGroups = [
    ...
    { label: 'Tempo & jornada', items: [
      { page: 'attendance', label: 'Ponto & assiduidade', icon: Clock3 },
      { action: 'attendance', label: 'Picagem móvel', icon: Clock3 },
      { action: 'panel', panel: 'rules', label: 'Regras de jornada', icon: Zap },
    ]},
    { label: 'Pessoas & RH', items: [
      { page: 'recruitment', label: 'Recrutamento', icon: BriefcaseIcon },
      { page: 'onboarding', label: 'Onboarding', icon: UserCheck },
    ...
    { label: 'Controlo', items: [
      { action: 'panel', panel: 'analytics', label: 'People Analytics', icon: Sparkles },
      { action: 'panel', panel: 'approvals', label: 'Aprovações', icon: CheckCircle2 },
      { page: 'alerts', label: 'Alertas', icon: AlertTriangle },
      ...(canManage ? [{ page: 'payroll', label: 'Folha', icon: CalendarDays }] : []),

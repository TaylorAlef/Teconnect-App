...
export default function AttendanceWorkspace({ profile, locations, anomalies, notify, onReload }) {
  ...
  const load = async (silent = false) => {
    ...
    try {
      const jobs = [supabase.rpc('get_my_clock_state')];
      if (canMonitor) jobs.push(supabase.rpc('get_company_attendance_live'));
      const results = await Promise.all(jobs);
      if (results[0].error) throw results[0].error;
      const nextClock = results[0].data || null;
      setClock(nextClock);
      if (canMonitor) {
        if (results[1].error) throw results[1].error;
        setLive(results[1].data || []);
      }
    } catch (error) {
      console.error(error);
  ...
  useEffect(() => { if (locations[0] && !selected) setSelected(locations[0].id); }, [locations, selected]);
  useEffect(() => { load(); const timer = window.setInterval(() => load(true), 15000); return () => window.clearInterval(timer); }, [profile?.company_id, canMonitor]);

  const punch = async (eventType) => {
    if (!location) {
      notify('Cadastre um local ativo com coordenadas GPS antes de marcar o ponto.', 'error');
      return;
    }
    setBusy(true);
    setLast(null);
    try {
      ...
      const { data, error } = await supabase.rpc('register_time_entry', {
        ...
        p_work_location_id: location.id,
        p_latitude: latitude,
        p_longitude: longitude,
        p_gps_accuracy: accuracy,
        p_device: navigator.userAgent.slice(0, 160),
      });
  ...
  return <>
    <section className="tc-hero">
      <div>
        <div className="tc-eyebrow"><LocateFixed size={13} /> Ponto & Geofence</div>
        <h1>Assiduidade operacional</h1>
        <p>O colaborador marca no dispositivo, o GPS é validado no cliente e o servidor confirma a zona autorizada antes de gravar.</p>
      </div>
      <div className="tc-actions"><button className="tc-btn" onClick={() => load()} disabled={loading}><RefreshCw size={15} /> Atualizar</button></div>
    </section>
    ...
    {clock?.employee && <section className="tc-two tc-section">
      <div className="tc-card tc-clock-card" style={{ minHeight: 320 }}>
        ...
        <div className="tc-clock-value">{today.toLocaleTimeString('pt-PT', { hour: '2-digit', minute: '2-digit' })}</div>
      ...
      <div className="tc-card tc-card-pad">
        <div className="tc-section-head"><div><h2>Resumo de hoje</h2><span>{attendance?.work_date || 'Ainda sem dia calculado'}</span></div><ShieldCheck size={18} className="tc-ok" /></div>
        <div className="tc-grid-6" style={{ gridTemplateColumns: 'repeat(2,1fr)', marginTop: 12 }}>
          <Mini label="Horário" value={shift ? `${String(shift.start_time).slice(0,5)}–${String(shift.end_time).slice(0,5)}` : '—'} />
          <Mini label="Trabalhado" value={fmtMinutes(attendance?.worked_minutes)} />
          <Mini label="Atraso" value={fmtMinutes(attendance?.late_minutes)} />
          <Mini label="Extra" value={fmtMinutes(attendance?.overtime_minutes)} />
        </div>
        <div className="tc-geofence" style={{ marginTop: 14 }}><MapPin size={16} /><span>{location ? `${location.name} · raio ${location.gps_radius_m} m` : 'Sem local configurado'}</span></div>
      </div>
    </section>}

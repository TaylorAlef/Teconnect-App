...
export default function SetupWizard({ profile, onClose }) {
  ...
  const steps = useMemo(() => [
    ...
    { id: 3, title: 'Ponto', icon: MapPin, done: locations.some((location) => Number.isFinite(Number(location.latitude)) && Number.isFinite(Number(location.longitude))) && shifts.length > 0 },
  ...
  const addLocation = async () => {
    ...
    const radius = Number(locationForm.gps_radius_m);
    if (!name) { setMessage('Indique o nome do local de trabalho.'); return; }
    if (!Number.isFinite(latitude) || latitude < -90 || latitude > 90) { setMessage('A latitude é obrigatória e deve estar entre -90 e 90.'); return; }
    if (!Number.isFinite(longitude) || longitude < -180 || longitude > 180) { setMessage('A longitude é obrigatória e deve estar entre -180 e 180.'); return; }
    if (!Number.isInteger(radius) || radius < 50 || radius > 5000) { setMessage('O raio GPS deve estar entre 50 e 5000 metros.'); return; }
    setSaving(true);
  ...
  const complete = async () => { const hasGpsLocation = locations.some((location) => Number.isFinite(Number(location.latitude)) && Number.isFinite(Number(location.longitude))); const hasAttendance = hasGpsLocation && shifts.length > 0; if (!hasAttendance) { setMessage('Antes de concluir, configure pelo menos um local com latitude/longitude válidas e um turno.'); setStep(3); return; } const next = { ...checklist, company: true, structure: departments.length > 0 && positions.length > 0, attendance: true, people: employees.length > 0 }; await saveChecklist(next, true); };

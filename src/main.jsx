...
import { Activity, AlertTriangle, Award, CheckCircle2, CreditCard, FileUp, Link2, LockKeyhole, LogOut, Settings2, Shield, UsersRound, Clock3, UserRound, X, Send } from 'lucide-react';
  ...
  return (
    <>
      ...
      <div className="tc-product-chrome">
        <button type="button" className="tc-btn primary tc-billing-trigger" onClick={openAttendance} title="Abrir ponto e geofence real"><Clock3 size={16} /> Ponto real</button>
        ...
        {profile.role !== 'SUPER_ADMIN' && <button type="button" className="tc-btn ghost tc-billing-trigger" onClick={() => setPanel('requests')} title="Férias, ausências e horas extra"><Send size={16} /> Solicitar</button>}
        ...
        <button type="button" className="tc-btn ghost tc-billing-trigger" onClick={() => setPanel('privacy')} title="Privacidade e exportação"><Shield size={16} /> Privacidade</button>
        {canApprove && <button type="button" className="tc-btn primary tc-billing-trigger" onClick={() => setPanel('approvals')} title="Férias, ausências e horas extra"><CheckCircle2 size={16} /> Aprovações</button>}
        {canManageHr && <>
          ...
          <button type="button" className="tc-btn ghost tc-billing-trigger" onClick={() => setPanel('lifecycle')} title="Entradas e saídas de colaboradores"><LogOut size={16} /> Offboarding</button>
          ...
          <button type="button" className="tc-btn ghost tc-billing-trigger" onClick={() => setPanel('billing')} title="Faturamento"><CreditCard size={16} /> Faturamento</button>
        </>}
        {profile.role === 'SUPER_ADMIN' && <button type="button" className="tc-btn ghost tc-billing-trigger" onClick={() => setPanel('super-admin')} title="Administração global"><Shield size={16} /> Super Admin</button>}
      </div>
      ...
      {panel && <div style={{ position: 'fixed', inset: 0, zIndex: 100, overflow: 'auto', background: 'var(--tc-bg, #0b1020)', padding: '26px 28px 44px' }}><div style={{ maxWidth: 1400, margin: '0 auto' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}><div style={{ color: 'rgba(255,255,255,.58)', fontSize: 12, display: 'flex', gap: 8, alignItems: 'center' }}><Shield size={15} /> Área protegida · tenant</div><button type="button" className="tc-btn" onClick={() => setPanel(null)}><X size={16} /> Fechar</button></div>
        ...
        {!['requests','security','privacy','self-service','approvals','people360','performance','setup','rules','exceptions','analytics','integrations','employee-import','lifecycle','billing','super-admin','employee-access','payroll','audit'].includes(panel) && <div style={{ padding: 32, textAlign: 'center' }}>Área disponível no centro administrativo.</div>}
      </div></div>}
    </>
  );
}

...
import { Activity, ArrowRight, Bell, Building2, CheckCircle2, Clock3, FileText, RefreshCw, Settings2, Users, X, Zap } from 'lucide-react';
    ...
    <>
      ...
      {open && (
        <div role="dialog" aria-modal="true" aria-label="Centro de operações" style={{ position: 'fixed', inset: 0, zIndex: 120, background: 'rgba(2,6,23,.70)', backdropFilter: 'blur(10px)', overflow: 'auto', padding: '44px 20px' }}>
          <div style={{ width: 'min(920px, 100%)', margin: '0 auto', border: '1px solid rgba(255,255,255,.11)', borderRadius: 22, background: 'linear-gradient(180deg,#111a2a,#0b111d)', boxShadow: '0 30px 90px rgba(0,0,0,.42)', overflow: 'hidden' }}>
      ...
      <style>{`@keyframes tcSpin{to{transform:rotate(360deg)}} @media(max-width:850px){.tc-enterprise-metrics{grid-template-columns:1fr 1fr!important}} @media(max-width:650px){.tc-enterprise-panel{padding:16px!important}.tc-enterprise-actions{grid-template-columns:1fr 1fr!important}}`}</style>
    </>
  );
}

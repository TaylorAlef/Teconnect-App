-- v208: preserve the name entered during signup when bootstrapping the company admin.
...
begin
  ...
  select pr.company_id into v_existing_company from public.profiles pr where pr.id=v_user_id limit 1;
  if v_existing_company is not null then raise exception 'COMPANY_ALREADY_ASSOCIATED'; end if;
  ...
  values(v_name,nullif(trim(p_nif),''),nullif(trim(p_phone),''),nullif(trim(p_address),''),true,'trial',false)
  returning id into v_company_id;

  insert into public.profiles(id,company_id,full_name,role,active)
  values(v_user_id,v_company_id,v_full_name,v_role,true)
  ...
  limit 1;
  if v_plan_id is null then raise exception 'STARTER_PLAN_NOT_FOUND'; end if;

  if not exists(select 1 from public.subscriptions s where s.company_id=v_company_id) then
    insert into public.subscriptions(company_id,plan_id,status,max_employees,trial_ends_at,renewal_at)
    select v_company_id,sp.id,'trial',sp.max_employees,now()+interval '14 days',now()+interval '14 days'
    from public.subscription_plans sp

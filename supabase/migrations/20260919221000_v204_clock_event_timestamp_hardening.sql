-- v204: harden clock-event ordering and SECURITY DEFINER search_path.
-- Use clock_timestamp() per event and a deterministic tie-breaker to avoid
-- same-transaction event ordering ambiguity.
...
begin
  ...
  where te.employee_id=v_employee
  ...
  insert into public.time_entries(
    company_id,employee_id,work_location_id,event_type,occurred_at,
    latitude,longitude,gps_accuracy,device,validation_status
  ) values(
    v_company,v_employee,p_work_location_id,v_canonical,v_event_at,
    p_latitude,p_longitude,p_gps_accuracy,p_device,'VALID'
  ) returning id into v_id;

  perform public.recalculate_attendance_day(v_employee,(v_event_at at time zone 'Europe/Lisbon')::date);

  select to_jsonb(ad) into v_attendance
  from public.attendance_days ad
  where ad.employee_id=v_employee
    and ad.work_date=(v_event_at at time zone 'Europe/Lisbon')::date;

  return jsonb_build_object(
    'id',v_id,'status','VALID','event_type',v_canonical,
    'distance_m',round(v_distance::numeric,2),'radius_m',v_radius,
    'employee_id',v_employee,'attendance',v_attendance
  );
end;
$function$;
